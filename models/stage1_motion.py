...
class VQAutoEncoder(BaseModel):
    """ VQ-GAN model for 51-dim motion """

    def __init__(self, args):
        super().__init__()
        self.encoder = TransformerEncoder(args)
        self.decoder = TransformerDecoder(args, args.motion_dim)
    ...
    def forward(self, x, template=None):
        ...
        Args:
            x: motion input (B, T, 51)
            template: 可选，用于兼容性（不使用）
        
        Returns:
            dec: decoded motion (B, T, 51)
            emb_loss: embedding loss
            info: quantization info
        """
        # motion不需要template，直接编码解码
...
class TransformerEncoder(nn.Module):
  ...
  def __init__(self, args):
    super().__init__()
    self.args = args
    size = self.args.motion_dim  # 51 for motion
    dim = self.args.hidden_size
    self.motion_mapping = nn.Sequential(nn.Linear(size,dim), nn.LeakyReLU(self.args.neg, True))
  ...
  def forward(self, inputs):
    ...
    inputs = self.motion_mapping(inputs)
...
class TransformerDecoder(nn.Module):
  ...
  def __init__(self, args, out_dim, is_audio=False):
    ...
    self.motion_map_reverse = nn.Linear(args.hidden_size,out_dim)
  ...
  def forward(self, inputs):
    ...
    pred_recon = self.motion_map_reverse(decoder_features)
    return pred_recon
